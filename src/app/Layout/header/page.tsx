    ...
    <nav className={`bg-white py-2 lg:py-4 shadow-md z-10 relative`}>
      <div className="container pl-2 lg:px-4 mx-auto flex items-center justify-between ">
        ...
        <div className="lg:hidden flex items-center ">
            ...
            <div key={index} className="relative group ">
              ...
              {!item.submenu ? (
                ...
                <div ref={modalRef} className="">
                  ...
                  <div
                    className={`absolute left-0 mt-1 w-48 bg-white border transition-all duration-300 overflow-hidden shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-xl 
                                        ${
                                          submenu[index] ? "block" : "hidden"
                    ...
                    {submenuOpen &&
                      item.submenu.map((sub, subIndex) => (
                        <Link
                          key={subIndex}
                          href={sub.path}
                          className="block px-4 py-4 hover:border-l-4 transition border-primaryGreen text-gray-700 hover:bg-primaryBlue hover:text-white"
                        >
                          <span className={styles.navFontSubmenu}>
                            {sub.name}
                          </span>
                        </Link>
                      ))}
          ...
          <Link
            href="/cart"
            className={`flex lg:hidden items-center py-2  
                                text-gray-600 rounded hover:bg-primaryBlue hover:text-primaryBlue
        ...
        <div
          className={`absolute lg:static  top-16 left-0 w-full lg:w-auto bg-white lg:bg-transparent
                 ...
                 } lg:flex lg:items-center lg:space-x-4 scrollHeader `}
        >
          <div
            className={`flex lg:items-center  flex-col lg:flex-row divide-y-2 lg:divide-y-0 ${
              isMobile && "max-h-[600px] overflow-y-auto overflow-x-hidden"
            } px-5 `}
          >
            {menuItems.map((item, index) => (
              <div key={index} className={`relative group `}>
                ...
                {!item.submenu ? (
                  <Link
                    href={item.path}
                    className={`flex items-center  gap-2 px-4 py-4 xl:px-4 lg:mx-2  text-gray-600 rounded lg:hover:bg-primaryBlue lg:hover:text-white transition-colors duration-300 ${
                      item.className || ""
                    }`}
                  >
                    <span className={styles.navFont}>{item.name}</span>
                  ...
                  <div ref={modalRef} className={item.className || ""}>
                    ...
                    <div
                      className={`lg:absolute divide-y-2 lg:divide-y-0 lg:left-0 lg:mt-1 text-center lg:text-start min-w-[250px] mx-auto lg:w-48  lg:bg-white lg:border transition-all duration-300 overflow-hidden lg:shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-xl 
                                        ${
                                          submenuOpen === true && submenu[index]
                                            ? "block"
                                            : "hidden"
                                        } lg:group-hover:block `}
                    ...
                    >
                      {item.submenu.map((sub, subIndex) => (
                        <Link
                          key={subIndex}
                          href={sub.path}
                          className="block px-4 py-4 lg:hover:border-l-4 transition lg:border-primaryGreen text-gray-700 lg:hover:bg-primaryBlue lg:hover:text-white"
                        >
                          <span className={styles.navFontSubmenu}>
                            {sub.name}
                          </span>
                      ...
                      className={`absolute left-0 mt-1 w-48 bg-white border transition-all duration-300 overflow-hidden shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-xl 
                                        ${
                                          ...
                                          submenu2[index]
                                            ? "block"
                                            : "hidden"
                                        } lg:group-hover:block divide-y-2 `}
                    ...
                    >
                      {item.submenu.map((sub, subIndex) => (
                        <Link
                          key={subIndex}
                          href={sub.path}
                          className="flex flex-col px-1 py-2 transition  text-gray-700 "
                        >
                          <span
                            className={`${styles.navFontSubmenu} hover:bg-primaryBlue hover:text-white rounded-lg p-2`}
                          >
                            {sub.name}
                          </span>
                        </Link>
                      ...
                      <button
                        ...
                        className="flex text-start flex-col px-1 py-2 w-full transition  text-gray-700 "
                      >
                        <span
                          className={`${styles.navFontSubmenu} hover:bg-primaryBlue hover:text-white rounded-lg p-2`}
                        >
            ...
            <Link
              href="/cart"
              className={`hidden lg:flex items-center py-2
                                text-gray-600 rounded hover:bg-primaryBlue hover:text-primaryBlue
                                 transition-colors duration-300 border border-transparent hover:bg-transparent px-0`}
            >
              <div className={`relative cursor-pointer`}>
